...
// Re-export Macrocategoria model so consumers of MenuRepository (including tests)
// can reference the type without importing the model directly.
...
class MenuRepository {
  ...
  FirebaseFirestore get _firestore => _firestoreOverride ?? core_fb.FirebaseService().firestore;
  ...
  /// Backwards-compatible: fetch macrocategorie as a one-shot future (used by tests)
  Future<List<Macrocategoria>> fetchMacrocategorie() async {
    final snapshot = await _macrocategorieCollection.orderBy('ordine').get();
...
/// Backwards-compatible alias so legacy code/tests expecting
/// `MenuRepositoryFirestore(firestore: ..)` keep working.
class MenuRepositoryFirestore extends MenuRepository {
  MenuRepositoryFirestore({required FirebaseFirestore firestore})
