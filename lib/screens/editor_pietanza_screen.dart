...
class _EditorPietanzaScreenState extends State<EditorPietanzaScreen> {
  ...
  void initState() {
    super.initState();
    // Defer async loading of categories until after first frame so
    // Firebase/menu services have a chance to initialize.
  ...
  // compatibility: Pietanza provides haFoto getter
  _usaFoto = p.haFoto;
      _categoriaSelezionataId = p.categoriaId;
      _categoriaSelezionataNome = p.categoria;
    } else {
      _immagineController.text = '🍽️';
      // we'll pick a default category after categories load in
      // _loadCategorieAsync(); if needed keep temporary placeholders
      _categoriaSelezionataId = null;
      _categoriaSelezionataNome = null;
    }
  }

  /// Async loader that ensures the MenuService has initialized and then
  /// refreshes the local categories list and the selected category.
  Future<void> _loadCategorieAsync() async {
    try {
      // initialize menu (safe to call repeatedly)
      await FirebaseService.menu.inizializzaMenu();
    } catch (_) {
      // ignore - menu service will have fallback data if needed
    }

    final serverCats = FirebaseService.menu.categorieMenu;
    setState(() {
      _tutteLeCategorie.clear();
      _tutteLeCategorie.addAll(serverCats);

      // If no category already selected (new item), pick a sensible default
      if ((_categoriaSelezionataId == null || _categoriaSelezionataId == 'default') && _tutteLeCategorie.isNotEmpty) {
        final defaultCat = _tutteLeCategorie.firstWhere(
          (c) => c.nome == widget.categoriaDefault,
          orElse: () => _tutteLeCategorie.first,
        );
        _categoriaSelezionataId = defaultCat.id;
        _categoriaSelezionataNome = defaultCat.nome;
      }
    });
  }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        final nuovaPietanza = Pietanza(
          ...
          // Use emoji when not using photo; otherwise set imageUrl
          emoji: !_usaFoto ? (_immagineController.text.isNotEmpty ? _immagineController.text : null) : null,
          imageUrl: _usaFoto ? (_immagineController.text.isNotEmpty ? _immagineController.text : null) : null,
          ingredienti: const [],
          ...
          categoriaId: _categoriaSelezionataId,
