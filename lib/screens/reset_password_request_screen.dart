...
class _ResetPasswordRequestScreenState extends State<ResetPasswordRequestScreen> {
  ...
  final token = await FirebaseService().clientAuth.generateResetToken(telefono);
      final sent = await EmailService.sendPasswordResetTokenEmail(
        toEmail: email,
      ...
      if (mounted) {
        if (sent) {
          messenger.showSnackBar(const SnackBar(content: Text('Email inviata. Controlla la tua posta.')));
          nav.pop();
        } else {
          messenger.showSnackBar(const SnackBar(content: Text('Errore invio email')));
        }
      }
    } catch (e) {
