...
class Pietanza {
  ...
  // Compat: ordine per visualizzazione nelle liste (legacy)
  final int ordine;
  ...
  Pietanza({
    ...
    String? fotoUrl,
    String? categoria,
    String? allergeniCompat,
    bool? usaFoto,
  }) :
    // map legacy params into canonical fields
    emoji = emoji ?? immagine,
    imageUrl = imageUrl ?? fotoUrl,
    ordine = ordine ?? 0,
    ingredienti = ingredienti ?? const [],
    allergeni = allergeniCompat != null
        ? (allergeniCompat.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList())
        : (allergeni ?? const []);
  ...
  factory Pietanza.fromMap(Map<String, dynamic> map) {
    return Pietanza(
      ...
      // Stato: be permissive with stored representations.
      // Accept: int index, string (either numeric index or enum name), or map with 'index'/'name'.
      stato: (() {
        final raw = map['stato'];
        try {
          if (raw is int) {
            if (raw >= 0 && raw < StatoPietanza.values.length) return StatoPietanza.values[raw];
          } else if (raw is String) {
            final asInt = int.tryParse(raw);
            if (asInt != null && asInt >= 0 && asInt < StatoPietanza.values.length) {
              return StatoPietanza.values[asInt];
            }
            final rawLower = raw.toLowerCase();
            for (final v in StatoPietanza.values) {
              if (v.toString().split('.').last.toLowerCase() == rawLower) return v;
            }
          } else if (raw is Map) {
            if (raw.containsKey('index') && raw['index'] is int) {
              final i = raw['index'] as int;
              if (i >= 0 && i < StatoPietanza.values.length) return StatoPietanza.values[i];
            }
  ...
  // Alcuni widget e adapter legacy si aspettano campi con nomi diversi
  // Forniamo dei getter compatibili per evitare di cambiare tutti i file UI

  /// Compat: nome usato in vecchi widget per categoria (stringa)
  String get categoria => categoriaId ?? macrocategoriaId;

  /// Compat: immagine (emoji o placeholder) utilizzata in UI legacy
  String get immagine => emoji ?? (imageUrl != null && imageUrl!.isNotEmpty ? '🖼️' : '');

  /// Compat: url della foto (vecchio nome "fotoUrl")
  String? get fotoUrl => imageUrl;

  /// Compat: indica se usare la foto invece dell'emoji
  bool get usaFoto => imageUrl != null && imageUrl!.isNotEmpty;

  /// Compat: quantita (usato in alcuni widget); default 0
  int get quantita => 0;
}