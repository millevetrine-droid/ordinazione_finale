...
class ArchivioCucinaScreen extends StatelessWidget {
  const ArchivioCucinaScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('📚 ARCHIVIO CUCINA'),
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        stream: FirebaseService.archive.getArchivioCucina(),
            ...
            return const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 20),
                  Text(
                    'Nessuna pietanza in archivio',
                    style: TextStyle(fontSize: 18, color: Colors.grey),
                  ),
                  SizedBox(height: 10),
                  Text(
                    'Le pietanze pronte archiviate appariranno qui',
  ...
  void _confermaRipristino(BuildContext context, String archivioId, String nomePietanza) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Vuoi ripristinare "$nomePietanza" in cucina?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
  ...
  void _ripristinaPietanza(BuildContext context, String archivioId, ScaffoldMessengerState messenger) async {
    try {
      await FirebaseService.archive.ripristinaPietanzaDaArchivio(archivioId);

      messenger.showSnackBar(
        const SnackBar(
          content: Text('✅ Pietanza ripristinata in cucina!'),
          backgroundColor: Colors.green,
