...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _onOrdinaOraPressed() {
    if (_currentTavolo != null) {
      ...
      // Nessun tavolo selezionato: mostra selezione tavolo invece del QR
      _selezionaTavoloTest(context);
    }
  }
  ...
  void _onTabTapped(int index) {
    ...
    if (index == 3) {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) => PointsDashboardScreen(numeroTavolo: _currentTavolo ?? '1'),
        ),
      );
      return;
    }
    if (index == 4) {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) => ProfileScreen(numeroTavolo: _currentTavolo ?? '1'),
        ),
      );
      return;
    }
