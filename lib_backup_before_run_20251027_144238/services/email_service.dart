...
class EmailService {
  ...
  // 👇 INVIO EMAIL CON LINK/TOKEN DI RESET (NON INVIARE PASSWORD IN CHIARO)
  static Future<bool> sendPasswordResetTokenEmail({
    required String toEmail,
    required String customerName,
    required String resetToken,
    required String appBaseUrl, // es. https://example.com/reset-password
  }) async {
    try {
      if (_resendApiKey.startsWith('re_xxx')) {
        dev.log('❌ ATTENZIONE: API Key non configurata!', name: 'EmailService');
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'html': '''
            ...
            <head>
                <meta charset="utf-8">
                <style> body { font-family: Arial, sans-serif; } .btn { background: #4CAF50; color: white; padding: 12px 20px; border-radius: 6px; text-decoration: none; } </style>
            </head>
            <body>
              <h2>Ciao $customerName</h2>
              <p>Hai richiesto il reset della password. Clicca il pulsante qui sotto per impostare una nuova password. Il link scadrà a breve.</p>
              <p><a class="btn" href="$resetUrl">Imposta nuova password</a></p>
      ...
      dev.log('❌ Errore invio reset token email: $e', name: 'EmailService');
      return false;
    }
  }
}