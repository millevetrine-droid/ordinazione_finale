...
class _CucinaScreenState extends State<CucinaScreen> {
        ...
        builder: (context, snapshot) {
          ...
          final ordiniConPietanzeAttive = ordini.where((ordine) {
            return ordine.pietanze.any((p) => 
              p.stato == 'in_attesa' || p.stato == 'in_preparazione'
            );
          }).toList();
  ...
  Widget _buildPietanzaItem(String ordineId, PietanzaOrdine pietanza) {
    return Card(
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('• ${pietanza.nome} x${pietanza.quantita}', style: const TextStyle(fontWeight: FontWeight.bold)),
                  Text('€ ${(pietanza.prezzo * pietanza.quantita).toStringAsFixed(2)} - ${_getStatoText(pietanza.stato)}'),
                ],
              ),
            ),
            if (pietanza.stato == 'in_attesa')
              ElevatedButton(
                onPressed: () => _cambiaStatoPietanza(ordineId, pietanza.idPietanza, 'in_preparazione'),
                style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, foregroundColor: Colors.white),
                child: const Text('INIZIA'),
              ),
            if (pietanza.stato == 'in_preparazione')
              ElevatedButton(
                onPressed: () => _cambiaStatoPietanza(ordineId, pietanza.idPietanza, 'pronto'),
